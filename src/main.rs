...
fn main() {
    ...
    let send_handler = future::loop_fn((), move |_| {
        ...
        // Delay makes the loop run just 2 times a second
        tokio::timer::Delay::new(Instant::now() + Duration::from_millis(500))
            ...
            .and_then(move |_| {
                ...
                for id in ids.iter() {
                    ...
                    // Serialize entitites
                    let entities = entities_inner.read().unwrap().clone();
                    let serial_entities = serde_json::to_string(&entities).unwrap();
                    //println!("Sending msg '{}'", serial_entities);
...
fn process_message(
    ...
    if let OwnedMessage::Text(ref txt) = *msg {
        ...
        println!("components: {:?}", message.components);
        ...
        for component in message.components {
            ...
            // TODO - Update only components sent instead of overwriting all
            // TODO - Instead of updating game state every frame, just emit latest data?
        }
